...
// Callback du serveur HTTP sur le GET d'une tuile :
// + rendre la tuile trouvee dans la base local mbtiles si elle existe,
// + sinon rendre cette tuile d'un serveur du Web
function zxy(mbtiles, req, res, next) {
        ...
        if (err) {
            ...
            if (redirect) {
                ...
                webTile(weburl, function(img,hdr) {
                    if (dbg) console.log("WEBcbsta img.length " + img.length + "\n" + hdr);
                    //var h2 = {'content-type': hdr['content-type'],
                    //          'etag': hdr['etag'],
                    //          'last-modified': 'Mon, 31 Oct 2016 16:16:45 GMT'};
                    res.header(hdr);
...
var mbtFile = "C:/GIS/NZ/NZ_OTM_TA_NI.mbtiles";             // Z=5..15
var mbtFile = "C:/GIS/NZ/NZ_South_Island.mbtiles";          // Z=8..14
var mbtFile = "C:/GIS/NZ/NZ_caltopo_SI.mbtiles";            // Z=5..15
var mbtFile = "C:/GIS/NZ/NZcaltopo12_TAocm15.mbtiles";      // Z=5..12, et 13..15 autour du TA
...
var mbts = new MBTiles(mbtFile, function(err, mbtiles) {
    ...
    srvTile.get('/tiles/:z/:x/:y.:e', function(req, res, next) {
        zxy(mbtiles, req, res, next); });

    srvTile.get('/:z/:x/:y', function(req, res, next) {     // URI de http://map.kd2.org/
        req.params.e = "png";
        zxy(mbtiles, req, res, next);    });
});
...
// Pour http://map.kd2.org/
srvHtml.listen(6090, function () {
    console.log(`Server running at http://localhost:6090/`);
});
